...
(function (root, window, document, factory, undefined) {
  ...
  TavoCalendar.prototype.mount = function () {
    ...
    calendar_nav_prev_el.innerHTML =
    ...
    for (var d = 1; d <= days_in_month; d++) {
      ...
      if (this.state.blacklist.indexOf(moment_copy.format(this.config.format)) > -1) {
        day_wrapper_el.className =
          day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_OFF + " " + CLASS_CALENDAR_DAY_LOCK;
      }
      ...
      if (this.state.date_start && moment_copy.isSame(this.state.date_start, "day")) {
        day_wrapper_el.className =
          ...
          " " +
          CLASS_CALENDAR_DAY_RANGE_SELECT +
          " " +
          CLASS_CALENDAR_DAY_SELECT;
      }

      if (
        this.state.date_start &&
        !this.state.date_end &&
        moment_copy.isBefore(moment(this.state.date_start, this.config.format), "day")
      ) {
      ...
      if (
        this.state.date_start &&
        this.state.date_end &&
        moment_copy.isBetween(
          moment(this.state.date_start, this.config.format),
          moment(this.state.date_end, this.config.format),
          "day"
        )
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_SELECT + " " + CLASS_CALENDAR_DAY_RANGE_SELECT;
      }

      //Lock days
      if (
        (moment_copy.isBefore(moment(this.state.date, this.config.format), "day") && !this.config.past_select) ||
        (moment_copy.isAfter(moment(this.state.date, this.config.format), "day") && !this.config.future_select)
      ) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_LOCK;
      }
  ...
  TavoCalendar.prototype.bindEvents = function () {
    ...
    this.elements.calendar_code.addEventListener(
      "click",
      function (ev) {
        ev.preventDefault();
